...
public abstract class TriggerAttackInstance : MonoBehaviour, IHitteableAttackInstance
{
    ...
    public virtual void Initialise(IEnumerable<IAttackControllerInfoProvider> attackControllerInfoProviders)
    {
        foreach (var provider in attackControllerInfoProviders)
        {
            ...
            DisableTriggerWithIgnored(_attackDamageablesInfoProvider.IgnoredDamageables);
        }
    }
    ...
    protected bool CheckTrigger(GameObject collisionGameObject)
    {
    ...
    protected abstract void DisableTriggerWithIgnored(IEnumerable<IDamageable> damageables);
}